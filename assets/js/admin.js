...
let allUsers        = []; // cache [{uid, name, role, email}]
...
function renderItem(d) {
  ...
  // Bandeau "Pris en charge" OU "Affecté à"
  let takenInfo = '';
  if (t.assignedTo) {
  ...
  const editBtn = window.__isAdmin
    ? `<button type="button" class="btn btn-outline-primary btn-sm" title="Modifier" data-edit="${id}">
         <i class="bi bi-pencil"></i>
       </button>`
    : '';

  const deleteBtn = window.__isAdmin
    ? `<button type="button" class="btn btn-outline-danger btn-sm" title="Supprimer" data-delete="${id}">
         <i class="bi bi-trash"></i>
       </button>`
    : '';

  const historyBtn = `<button type="button" class="btn btn-link btn-sm p-0 mt-2 text-decoration-none"
    data-history="${id}" title="Voir l'historique">
      ...
      <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
        <div style="flex: 1;">
          ...
          <h5 class="card-title mb-1 d-flex align-items-center flex-wrap gap-2">
            <span class="badge bg-secondary fw-normal font-monospace user-select-all"
                  style="font-size:.70rem; letter-spacing:.02em;" title="ID complet : ${id}">
              #${id}
            </span>
...
function refreshList() {
  ...
  // Rafraîchissement async des noms takenBy
  setTimeout(async () => {
  ...
  try {
    ...
    snap.forEach(ds => {
      ...
      if (h.field === 'title' || h.field === 'description') {
        ...
        changeDisplay = `<div class="mt-1">
          ${h.oldValue
            ? `<span class="badge bg-secondary me-2">${h.oldValue}</span>`
            : '<span class="text-muted me-2">(nouveau)</span>'}
 ...
 * et retourne un tableau [{uid, name, role, email}] dédupliqué, trié alpha.
 */
...
/** Affiche la liste filtrée dans le modal d'affectation */
function renderAssignList(users, currentAssignedUid = null) {
  ...
  listEl.innerHTML = users.map(u => {
    ...
    return `
      <button type="button"
              class="list-group-item list-group-item-action d-flex justify-content-between align-items-center py-2 ${isActive ? 'active' : ''}"
              data-assign-uid="${u.uid}"
              data-assign-name="${u.name}">
        <span>
          <i class="bi bi-person-circle me-2 opacity-50"></i>
          <strong>${u.name}</strong>
          ${u.email ? `<small class="${isActive ? '' : 'text-muted'} ms-2">${u.email}</small>` : ''}
        </span>
...
// Clic sur un utilisateur → affecter
...
document.getElementById('btn-unassign')?.addEventListener('click', async () => {
  ...
  try {
    const ticketRef = doc(db, 'tickets', currentAssignId);
    const oldName   = (await getDoc(ticketRef)).data()?.assignedTo || '?';
